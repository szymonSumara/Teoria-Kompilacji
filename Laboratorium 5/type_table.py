...
def get_new_symbol(op, left, right):
    if isinstance(left, VectorSymbol):
        ...
        if op == '.*' or op == './' or op == '.+' or op == '.-':
            ...
            return 'Can\'t {} Vector to {}'.format(op, right.type_name)
        ...
        if op == '==' or op == '!=':
            if isinstance(right, VectorSymbol):
                ...
                return 'Can\'t {} Vector of type {} with Vector of type {}'.format(op, left.type, right.type)
            return 'Can\'t {} Vector to {}'.format(op, right.type_name)
    ...
    if isinstance(left, MatrixSymbol):
        ...
        if op == '.*' or op == './' or op == '.+' or op == '.-':
            ...
            return 'Can\'t {} Matrix with {}'.format(op, right.type_name)
        ...
        if op == '==' or op == '!=':
            if isinstance(right, MatrixSymbol):
                ...
                if new_type:
                    return VariableSymbol('Variable', new_type)
                return 'Can\'t {} Matrix of type {} with Matrix of type {}'.format(op, left.type, right.type)
            return 'Can\'t {} Matrix to {}'.format(op, right.type_name)
    ...
    if isinstance(left, VariableSymbol):
        ...
        return 'Can\'t {} Variable with {}'.format(op, right.type_name)
